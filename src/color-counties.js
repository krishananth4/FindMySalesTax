const allcounties = [
 ...
 },
 {
   "state": "Puerto Rico",
   "code": 72131,
   "county": "San Sebastián Municipio"
 },
 {
   "state": "Oregon",
   "code": 41019,
...
function getHeatmapColor(taxRate, minRate = 0, maxRate = 15) {
    if (taxRate === null || taxRate === undefined) {
        return '#cccccc'; // Gray for no data
    }
    
    // normalize the tax rate to 0-1 range
    const normalized = Math.min(Math.max((taxRate - minRate) / (maxRate - minRate), 0), 1);
    
    // (low) Blue -> Green -> Yellow -> Red (high)
    let r, g, b;
    ...
    if (normalized < 0.33) {
        ...
        const factor = (normalized - 0.33) / 0.33;
        r = Math.round(255 * factor);
        g = 255;
        b = 0;
    } else {
        const factor = (normalized - 0.66) / 0.34;
        r = 255;
...
async function applyHeatmapToDatabase(locations, options = {}) {
    const {
        fillOpacity = 0.7,
        strokeWidth = 1,
        strokeColor = '#333',
        minTaxRate = 0,
        maxTaxRate = 15,
        delayBetweenRequests = 100, // ms delay to avoid overwhelming server
        batchSize = 10 // Process in batches to avoid too many concurrent requests
    } = options;
    
    console.log(`Starting heatmap processing for ${locations.length} locations...`);
    
    // Store all tax rates first to determine actual min/max if needed
    const taxRates = [];
    const results = [];
    ...
    for (let i = 0; i < locations.length; i += batchSize) {
        ...
        const batchPromises = batch.map(async (location, batchIndex) => {
            ...
            try {
                const taxRate = await getSalesTax([location.county, location.state]);
                
                if (taxRate !== null) {
                    taxRates.push(taxRate);
                }
                let id;
                if (location.code.toString().length == 4) {
                    id="c0"+location.code;
                }else{
                    id="c"+location.code;
                }
                ...
                console.error(`Error processing ${location.county}, ${location.state}:`, error);
                return {
                    index: globalIndex,
                    location: location,
                    taxRate: null,
        ...
        console.log(`Processed batch ${Math.floor(i/batchSize) + 1}/${Math.ceil(locations.length/batchSize)}`);
        
        // Add delay between batches if specified
        if (delayBetweenRequests > 0 && i + batchSize < locations.length) {
            await new Promise(resolve => setTimeout(resolve, delayBetweenRequests));
        }
    }
    
    // Calculate actual min/max if auto-scaling is desired
    const actualMinRate = Math.min(...taxRates);
    const actualMaxRate = Math.max(...taxRates);
    const useMinRate = options.autoScale ? actualMinRate : minTaxRate;
    const useMaxRate = options.autoScale ? actualMaxRate : maxTaxRate;
    
    console.log(`Tax rate range: ${useMinRate}% - ${useMaxRate}%`);
    ...
    results.forEach(result => {
        const { elementId, taxRate, location, error } = result;
        const element = document.getElementById(elementId) || document.querySelector(`g[data-county="${location.county}"][data-state="${location.state}"]`);
        
        if (element) {
            const color = getHeatmapColor(taxRate, useMinRate, useMaxRate);
            
            // Find the path element within the group to apply styling
            const pathElement = element.querySelector('path');
            if (pathElement) {
                // Apply styles to the path element instead of the group
                ...
                // Fallback: apply to the group element but preserve pointer events
                element.style.fill = color;
                element.style.fillOpacity = fillOpacity;
                element.style.stroke = strokeColor;
                element.style.strokeWidth = strokeWidth;
                // Ensure pointer events are preserved
                element.style.pointerEvents = 'auto';
            }
            
            // Add data attributes to the group element for reference
            element.setAttribute('tax-rate', taxRate || 'null');
            element.setAttribute('county', location.county);
            element.setAttribute('dstate', location.state);
            
            // Add title for hover tooltip to the group element
            ...
            console.warn(`Element not found for ${location.county}, ${location.state} (ID: ${elementId})`);
            errorCount++;
        }
        
        if (error) {
            console.error(`Error for ${location.county}, ${location.state}: ${error}`);
        }
    });
    
    console.log(`Heatmap processing complete. Success: ${successCount}, Errors: ${errorCount}`);
    
    return {
        processed: results.length,
        successful: successCount,
        errors: errorCount,
        taxRateRange: { min: useMinRate, max: useMaxRate },
        results: results
    };
}
...
applyHeatmapToDatabase(allcounties, {
    ...
    minTaxRate: 0,
    maxTaxRate: 12,
    autoScale: true, // Use actual min/max from data
